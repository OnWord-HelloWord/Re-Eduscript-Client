...
class FontColorDropdown extends StatelessWidget {
  const FontColorDropdown({super.key});

  @override
  Widget build(BuildContext context) {
    // [provider] 자막 스타일 받아오기 (폰트 색상)
    final styles = context.watch<SubtitleStyleProvider>();

    return CommonSettingDropdown(
      name: "색상",
      initialValue: styles.selectedFontColor, // 초기값
      optionList: styles.fontColorOptions,    // 옵션 리스트
      onChanged: (String newColor) {          // 상태 변화
        styles.updateFontColor(newColor);
      },
